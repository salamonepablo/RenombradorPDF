...
class ViajeLDRenamerApp:
    ...
    def setup_ui(self):
        ...
        tk.ttk.Label(control_frame, text="Nro. Tren:").grid(column=2, row=0, sticky=tk.W, padx=(0, 5))
    ...
    def _format_train_entry(self, event):
        ...
        if content.isdigit() and len(content) == 3:
            ...
            self.train_entry.insert(0, content)
    ...
    def rename_file(self):
        ...
        if not self.validate_train_number(train_input):
            tk.messagebox.showerror("Error", "Formato de Nro. Tren incorrecto.\nDebe ser un número entre 301 y 310.")
            return
        if date_str and not self.validate_date(date_str):
            ...
            return
        if date_str:
            new_name = f"{date_str} MPN {train_input}.pdf"
        else:
            today = datetime.now().strftime("%d-%m")
            new_name = f"{today} MPN {train_input}.pdf"
        try:
